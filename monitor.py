...
def monitor():
	while True:
		...
		try:
			inTerFacE=int(input("Choose the interface you want to enable monitormode: "))
			if inTerFacE <= lenList and inTerFacE != 0:
				inTERFACE=intErface[inTerFacE-1]
				def run_commnd(inTERFACE):
					subprocess.run(["xterm","-e","sudo ip link set "+ inTERFACE +" down;sleep 2s;sudo iw "+ inTERFACE  +" set type monitor;sleep 2s"])
					subprocess.run(["clear"])
